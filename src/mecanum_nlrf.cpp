...
void calcMotorPower(float degree,float length,int line) {
  /**
   * @param degree: -180~180
   * @param length: 0~1
   *@oparam line 0 or 1 1:use
   */
	float x,y,tn,dt,radian,c=48,t=25,vertical_g,horizonal_g;
	...
	//回転調整
	vertical_g=(line_param[0]+line_param[2]);
	horizonal_g=(line_param[1]+line_param[3]);
	dt=calcdt(vertical_g,horizonal_g);
...
void set(){
	...
	while(delaying){
		...
		calcMotorPower(set_degree,set_length,DO_NOT_USE_LINE);
	  	ros::spinOnce();
	  	loop_rate.sleep();
...
void startToPass1(){
  for(int i=0;i<10;i++){
	  calcMotorPower(-80, i*0.1, DO_NOT_USE_LINE);
	  ros::Duration(3).sleep();
  }
  delay(1000);
  while(delaying){
  	calcMotorPower(-80, 1, DO_NOT_USE_LINE);
  	ros::spinOnce();
  	loop_rate.sleep();
  	ros::Duration(0.1).sleep();
  	delayCount();
  ...
  for(int i=0;i<7;i++){
	  calcMotorPower(-80, 1-i*0.1, DO_NOT_USE_LINE);
	  ros::Duration(3).sleep();
  }
  while(line_param[1]!=0){
  	calcMotorPower(-80, 0.3, DO_NOT_USE_LINE);
  	ros::spinOnce();
  	loop_rate.sleep();
  }
} 
...
void pass1ToShot1(){
  for(int i=0;i<10;i++){\
	  calcMotorPower(RIGHT, i*0.1, USE_LINE);
	  ros::Duration(3).sleep();
  }
  delay(1000);
  while(delaying){
  	calcMotorPower(RIGHT, 1, USE_LINE);
  	ros::spinOnce();
  	loop_rate.sleep();
  	ros::Duration(0.1).sleep();
  	delayCount();
  ...
  for(int i=0;i<7;i++){
	  calcMotorPower(RIGHT, 1-i*0.1, USE_LINE);
	  ros::Duration(3).sleep();
  }
  while(line_param[0]!=0){
  	calcMotorPower(RIGHT, 0.3, USE_LINE);
  	ros::spinOnce();
  	loop_rate.sleep();
  }
...
void shot1ToPass2(){
 for(int i=0;i<10;i++){
	  calcMotorPower(LEFT, i*0.1, USE_LINE);
	  ros::Duration(3).sleep();
  }
  delay(1000);
  while(delaying){
  	calcMotorPower(LEFT, 1, USE_LINE);
  	ros::spinOnce();
  	loop_rate.sleep();
  	ros::Duration(0.1).sleep();
  	delayCount();
  ...
  for(int i=0;i<7;i++){
	  calcMotorPower(LEFT, 1-i*0.1, USE_LINE);
	  ros::Duration(3).sleep();
  }
  while(line_param[3]!=0){
  	calcMotorPower(LEFT, 0.3, USE_LINE);
  	ros::spinOnce();
  	loop_rate.sleep();
  }
}
...
void pass2ToShot2(){
  for(int i=0;i<10;i++){
	  calcMotorPower(-40, i*0.1, DO_NOT_USE_LINE);
	  ros::Duration(3).sleep();
  }
  delay(1000);
  while(delaying){
  	calcMotorPower(-40, 1, DO_NOT_USE_LINE);
  	ros::spinOnce();
  	loop_rate.sleep();
  	ros::Duration(0.1).sleep();
  	delayCount();
  ...
  for(int i=0;i<7;i++){
	  calcMotorPower(-40, 1-i*0.1, DO_NOT_USE_LINE);
	  ros::Duration(3).sleep();
  }
  
  while(line_param[1]!=0){
  	calcMotorPower(-40, 0.3, DO_NOT_USE_LINE);
  	ros::spinOnce();
  	loop_rate.sleep();
  }
